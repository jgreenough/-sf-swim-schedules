...
| M3 | Scheduler (daily refresh) | **DONE 2026-05-03.** Project pushed to GitHub repo (`jgreenough/sf-swim-schedules`); workflow `.github/workflows/refresh-schedules.yml` runs `parse_all.py` daily at 13:37 UTC and commits any changed `parsed/*.json` / `pdfs/*.pdf` back to the default branch as github-actions[bot]. Manual trial run via the Actions tab completed green end-to-end. From this point forward the data refreshes itself; an operator only needs to intervene if a workflow run fails (which surfaces as a GitHub email notification). |
...
| 2026-05-03 | M3 done. GitHub repo `jgreenough/sf-swim-schedules` created and project pushed; `.github/workflows/refresh-schedules.yml` workflow's manual trial run completed green end-to-end. Daily cron at 13:37 UTC takes over from here. Auth bumps along the way: the user has MFA on, so initial SSH push failed (no key registered) and HTTPS password auth is no longer accepted by GitHub for git operations; resolved by installing `gh` CLI and running `gh auth login` (browser-based OAuth handles MFA cleanly and configures git's credential helper) | _you_ |
