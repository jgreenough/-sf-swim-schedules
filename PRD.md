...
| M3 | Scheduler (daily refresh) | Per-owner direction during M3 kickoff: M3 was rescoped to **scheduler-only** (smaller increment); the `.ics` feed generation and the searchable web view were pushed to M4. **Implementation in progress 2026-05-03.** GitHub Actions workflow `.github/workflows/refresh-schedules.yml` runs `parse_all.py` daily at 13:37 UTC (5:37 AM Pacific Standard, 6:37 AM Pacific Daylight; minute-37 offset reduces queue contention with the hourly cron stampede), checks out the repo, installs dependencies from `requirements.txt`, runs the parser, and commits any changed `parsed/*.json` and `pdfs/*.pdf` files back to the default branch as the github-actions[bot] user. Hosting choice: GitHub Actions, free tier, runs on GitHub-managed Linux VMs (no dependency on the owner's Mac being on). New files: workflow YAML, `requirements.txt` (pdfplumber, pypdf), `.gitignore`. M3 done when the workflow has run successfully end-to-end at least once and is on the daily cron schedule. |
| M4 | Web + `.ics` | Filterable web page live; subscribable calendar feed live. (Bumped from M3 per scheduler-only M3 scoping.) |
