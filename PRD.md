...
| Area | Requirement |
|---|---|
| Refresh cadence | Re-fetch source PDFs once per week (per owner direction during M3). SF Rec & Park publishes on a seasonal cadence (every ~3 months), so weekly polling catches changes within 7 days of publication — acceptable for personal v1 use. Easily increased to daily later if needed by changing the cron in `.github/workflows/refresh-schedules.yml`. |
...
| M3 | Scheduler (daily refresh) | **DONE 2026-05-03.** Project pushed to its GitHub repo; workflow `.github/workflows/refresh-schedules.yml` runs `parse_all.py` weekly at 13:37 UTC every Monday and commits any changed `parsed/*.json` / `pdfs/*.pdf` back to the default branch as github-actions[bot]. Manual trial run via the Actions tab completed green end-to-end. From this point forward the data refreshes itself; an operator only needs to intervene if a workflow run fails (which surfaces as a GitHub email notification). |
| M4 | Web view (filterable schedule browser) | **DONE 2026-05-03.** Web-view-only scope (per kickoff). `index.html` at the repo root — plain HTML + vanilla JS, no build step or framework — loads `pools.json` and every active pool's `parsed/*.json` at page load, renders filter chips for swim type / day / pool plus a start-time range, syncs filter state to the URL hash for shareable links, and groups results by day-of-week with color-coded swim-type chips. Defaults to Lap Swim + Family Swim only (per §5.3). Mobile-responsive; supports light and dark mode via `prefers-color-scheme`. Hosted on GitHub Pages from the repo root. One post-deploy bug fixed: the pool filter's "empty set = all selected" toggle flipped sibling chips' filter state but not their visual `aria-pressed`, leaving other chips visually stale; resolved by re-evaluating active state for every chip in the group on each click. |
...
| 2026-05-03 | PII audit + scrub before flipping repo public. PRD references to GitHub username and to the auth-troubleshooting narrative removed. Git config set globally to use the noreply email. Two original commits (initial + M4) had author email `joshuagreenough2026@Joshuas-MacBook-Pro.local` (auto-generated from Mac username + hostname) — rewritten via `git filter-branch --env-filter` and force-pushed. Bot's commits unchanged. Verified zero hostname leaks across all commits. SF Rec & Park staff `@sfgov.org` emails still appear in `parsed/*_raw.json` debug files but those are public officials' work emails, already in the source PDFs we mirror — flagged as low-risk; can be gitignored later if leaner public face desired | _you_ |
| 2026-05-03 | Post-deploy fix to `index.html`: pool filter chips weren't visually toggling on/off when clicked even though the underlying filter state updated correctly. Root cause: the click handler updated `aria-pressed` only on the clicked chip, but the pool group's "empty set = all selected" multi-toggle behavior flipped sibling chips' active state too. Fixed by re-evaluating `isActive` for every chip in the group on each click. Same code path now serves all filter groups (types, days, pools) consistently. M4 marked done | _you_ |
| 2026-05-03 | Per owner direction, lowered scheduler cadence from daily to **weekly (Monday 13:37 UTC)**. SF Rec & Park publishes seasonally (~every 3 months), so weekly polling catches changes within 7 days of publication — acceptable for personal v1, less commit churn, fewer Actions minutes used. Cron in `.github/workflows/refresh-schedules.yml` changed from `'37 13 * * *'` to `'37 13 * * 1'`. §6 refresh-cadence requirement updated to match | _you_ |
| 2026-05-03 | Per owner direction, **merged `recreation_swim` into `family_swim`** with display label "Rec/Family Swim". Reasoning: SF Rec & Park itself often labels them as one cell ("REC/FAMILY SWIM"), and from the user's perspective they're the same thing — a public-drop-in swim. The `family_swim` regex now also covers `recreation swim`, `rec swim`, `open swim`, `safety swim`, and `splash`; the standalone `recreation_swim` rule and TYPE_META entry are removed. All parsed JSONs regenerated (56 family_swim sessions across active pools, 0 recreation_swim — merge complete). Web view filter row now has one fewer chip. Balboa M1 diff still passes 29/29; the expected fixture already used family_swim for the rec/family compound cell so no update needed | _you_ |
